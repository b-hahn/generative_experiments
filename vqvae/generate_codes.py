...
def extract_codes():
    ...
    # load params
    weights_file = Path('/home/ben/generative_experiments/vqvae/run_2022-02-01-11-09-48/weights_70000.pkl')
    ...
    if cfg.dataset == 'cifar':
        ...
        train_dataset = dataloader.get_swisstopo_dataset(split=split, img_w=128,
                                                            img_h=128).map(cast_and_normalise_images_swisstopo)
    else:
        raise RuntimeError(f'Unknown dataset {cfg.dataset}')
    # TODO: compute actual dataset variance
    train_data_variance = 0.01
    ...
    train_batch = jnp.asarray(next(sample_train_batch))
    _, state = forward_fn.init(subkey, train_batch, is_training=False, cfg=cfg)
    ...
    for step, train_batch in enumerate(train_dataset_iter):
        ...
        model_output, state = forward_fn.apply(params, state, subkey, jnp.asarray(train_batch), is_training=False, cfg=cfg)
        # model_output, state = forward_fn.apply(params, state, subkey, train_batch['image'], is_training=False, cfg=cfg)
        encodings = model_output['encodings'].reshape(train_batch.shape[0], -1, 64, 512)  # e.g. 32, 32, 64, 512
        ...
        encodings_numpy.append(np.array(encodings))
        # if len(encodings_numpy) * encodings.nbytes > 5e8:
            # np.save(encodings_dir / f"encodings_{split}_{chunk_counter}", np.concatenate(encodings_numpy))
            # encodings_numpy = []
            # chunk_counter += 1
        print(f"Processing {step}...", end="\r", flush=True)
    np.save(encodings_dir / f"encodings_{split}_{cfg.dataset}_{chunk_counter}", np.concatenate(encodings_numpy))
